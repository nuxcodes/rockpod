...
/* Uncacheable memory alias for DMA buffers (same as vpu_h264.c) */
...
/* JPEG zigzag → raster position mapping (for Q table de-zigzag) */
...
static void vpua_power_on(void)
{
    ...
    /* No delay: PLL2 already locked, CG16_SVID is a gate.
     * Apple FW + all Rockbox drivers use zero delay. */
...
enum plugin_status plugin_start(const void *parameter)
{
    ...
    /* Allocate VPU-A buffers from remaining audio buffer */
    {
        ...
        /* Load Q tables from JPEG into XFORM (de-zigzagged to raster order) */
        {
            int i;
            for (i = 0; i < 64; i++)
            {
